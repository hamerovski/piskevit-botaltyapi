...
//----------- YKS GERİ SAYIM ---------------------------------------------------------------------------------------------------------------------client.on("ready",async message => {
  ...
  const countdown = (param) => {
          ...
          const cd = () => {
            ...
            kanall.messages.fetch(mesajid).then(x => {
                x.edit(Discord.MessageEmbed().setColor("PURPLE").setDescription(`${(countdownTextNow)(param.output)(param.date)}`).setFooter("YKS SAYACI!"))
