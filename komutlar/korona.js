...
exports.run = async (client, message, args) => {
  if(!args[0]) return message.channel.send('```𝘾𝙊𝙍𝙊𝙉𝘼 𝙑𝙄𝙍𝙐𝙎 𝘼𝙉𝙇𝙄𝙆 𝙄𝙎𝙏𝘼𝙏𝙄𝙎𝙏𝙄𝙆 𝙏𝘼𝘽𝙇𝙊𝙎𝙐 𝙆𝙐𝙇𝙇𝘼𝙉𝙄𝙈𝙄```\n\n**Türkiye :flag_tr:**```p.korona TR```\**Çin :flag_cn:**```p.korona CN```\**Amerika :flag_us:**```p.korona US```\**Japonya :flag_jp:**```p.korona JP```\**Fransa :flag_fr:**```p.korona FR```\**Norveç :flag_ne:**```q!korona NE```\**İsveç :flag_se:**```q!korona SN```')
