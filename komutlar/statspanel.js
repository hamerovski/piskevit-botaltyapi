...
.then((collected) => { 
  ...
  message.guild.channels.create(`${client.user.username} • Sunucu Panel`, 'category', [{id: message.guild.id, deny: ['CONNECT']}]);
        message.guild.channels.create(`Toplam Üye • ${message.guild.members.cache.size}`, 'voice').then(channel => channel.setParent(message.guild.channels.cache.find(channel => channel.name === `${client.user.username} • Sunucu Panel`))).then(c => {
      c.createOverwrite(role, {
          CONNECT: false,
      });
  })
  
        message.guild.channels.create(`Aktif Üye • ${message.guild.members.cache.filter(off => off.presence.status !== 'offline').size}`, 'voice').then(channel => channel.setParent(message.guild.channels.cache.find(channel => channel.name === `${client.user.username} • Sunucu Panel`))).then(c => {
      c.createOverwrite(role, {
          CONNECT: false,
      });
  })
  
        message.guild.channels.create(`Botlar • ${message.guild.members.cache.filter(m => m.user.bot).size}`, 'voice').then(channel => channel.setParent(message.guild.channels.cache.find(channel => channel.name === `${client.user.username} • Sunucu Panel`))).then(c => {
      c.createOverwrite(role, {
          CONNECT: false,
      });
  })
  
        message.guild.channels.create(`Rekor Aktiflik • ${message.guild.members.cache.filter(off => off.presence.status !== 'offline').size}`, 'voice').then(channel => channel.setParent(message.guild.channels.cache.find(channel => channel.name === `${client.user.username} • Sunucu Panel`))).then(c => {
      c.createOverwrite(role, {
          CONNECT: false,
      });
  })
